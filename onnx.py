...
def load_onnx_model(detect_model, rec_model):
    providers = ['CPUExecutionProvider']
    session_detect = onnxruntime.InferenceSession(detect_model, providers=providers)
    session_rec = onnxruntime.InferenceSession(rec_model, providers=providers)
    return session_detect, session_rec


def rec_by_session(image, img_size, session_detect, session_rec):
    img, r, left, top = detect_pre_precessing(image, img_size)
    ...
    return rec_plate(outputs, image, session_rec)
